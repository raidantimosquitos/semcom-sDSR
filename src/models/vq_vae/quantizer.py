"""
Vector quantizers for VQ-VAE.

- VectorQuantizerEMA: EMA with commitment loss, used by VQ_VAE_2Layer
"""

from __future__ import annotations

import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.amp.autocast_mode import autocast
from typing import Tuple

class VectorQuantizerEMA(nn.Module):
    """
    EMA vector quantizer with commitment cost (VQ-VAE style).
    Returns (loss, quantized, perplexity, encoding_indices).
    Reference: https://github.com/zalandoresearch/pytorch-vq-vae
    """

    def __init__(
        self,
        num_embeddings: int,
        embedding_dim: int,
        commitment_cost: float,
        decay: float,
        epsilon: float = 1e-5,
        distance_chunk_size: int = 256,
    ) -> None:
        super().__init__()
        self._num_embeddings = num_embeddings
        self._embedding_dim = embedding_dim
        self._commitment_cost = commitment_cost
        self._decay = decay
        self._epsilon = epsilon
        self._distance_chunk_size = int(distance_chunk_size)

        self._embedding = nn.Embedding(num_embeddings, embedding_dim)
        self._embedding.weight.data.normal_()

        self.register_buffer("_ema_cluster_size", torch.zeros(num_embeddings))
        self._ema_w = nn.Parameter(torch.empty(num_embeddings, embedding_dim))
        self._ema_w.data.normal_()

    def _nearest_embedding_indices(self, flat_input: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        """
        Compute nearest codebook index for each row in flat_input without
        materializing a full (N, K) distance matrix.

        Args:
            flat_input: (N, D) float32
            emb: (K, D) float32

        Returns:
            indices: (N,) long
        """
        N = int(flat_input.shape[0])
        K = int(emb.shape[0])
        if K <= 0 or N <= 0:
            return torch.empty((N,), device=flat_input.device, dtype=torch.long)

        chunk = max(1, min(int(self._distance_chunk_size), K))
        x2 = flat_input.pow(2).sum(dim=1, keepdim=True)  # (N, 1)

        best_d = torch.full((N,), float("inf"), device=flat_input.device, dtype=torch.float32)
        best_i = torch.zeros((N,), device=flat_input.device, dtype=torch.long)

        offset = 0
        while offset < K:
            end = min(offset + chunk, K)
            e = emb[offset:end]  # (kc, D)
            d = x2 + e.pow(2).sum(dim=1).unsqueeze(0) - 2.0 * (flat_input @ e.t())  # (N, kc)
            d_min, d_arg = d.min(dim=1)
            upd = d_min < best_d
            if upd.any():
                best_d = torch.where(upd, d_min, best_d)
                best_i = torch.where(upd, d_arg + offset, best_i)
            offset = end

        return best_i

    def forward(
        self, inputs: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Returns:
            loss: commitment loss (scalar)
            quantized: quantized tensor (B, C, H, W)
            perplexity: codebook utilization metric
            encoding_indices: (N,) long indices into the codebook

        Runs in float32 under AMP to avoid NaN from distance computation and EMA updates.
        """
        input_dtype = inputs.dtype
        # Run entire quantizer in float32 for numerical stability (AMP-safe)
        with autocast(device_type=inputs.device.type, enabled=False):
            inputs = inputs.float()
            inputs = inputs.permute(0, 2, 3, 1).contiguous()
            input_shape = inputs.shape
            flat_input = inputs.view(-1, self._embedding_dim)

            # Embedding in float32 for stable distances and EMA
            emb = self._embedding.weight.float()
            encoding_indices = self._nearest_embedding_indices(flat_input, emb)  # (N,)
            quantized = emb[encoding_indices].view(input_shape)  # (B, H, W, C)

            if self.training:
                counts = torch.bincount(encoding_indices, minlength=self._num_embeddings).to(torch.float32)
                self._ema_cluster_size.mul_(self._decay).add_(counts, alpha=1.0 - self._decay)
                n = self._ema_cluster_size.sum()
                self._ema_cluster_size.data = (
                    (self._ema_cluster_size + self._epsilon)
                    / (n + self._num_embeddings * self._epsilon) * n
                )

                dw = torch.zeros_like(self._ema_w.data, dtype=torch.float32)
                dw.index_add_(0, encoding_indices, flat_input)
                self._ema_w.data.mul_(self._decay).add_(dw, alpha=1.0 - self._decay)
                self._embedding.weight.data.copy_(
                    self._ema_w / self._ema_cluster_size.unsqueeze(1).clamp(min=1e-5)
                )
                

            e_latent_loss = F.mse_loss(quantized.detach(), inputs)
            loss = self._commitment_cost * e_latent_loss

            quantized = inputs + (quantized - inputs).detach()
            avg_probs = torch.bincount(encoding_indices, minlength=self._num_embeddings).to(torch.float32)
            avg_probs = avg_probs / avg_probs.sum().clamp(min=1.0)
            perplexity = torch.exp(-(avg_probs * (avg_probs + 1e-10).log()).sum())

            quantized = quantized.permute(0, 3, 1, 2).contiguous()
            if input_dtype != torch.float32:
                quantized = quantized.to(input_dtype)
            loss = loss.to(input_dtype)
            perplexity = perplexity.to(input_dtype)

        return (
            loss,
            quantized,
            perplexity,
            encoding_indices,
        )

    def get_indices(self, inputs: torch.Tensor) -> torch.Tensor:
        """
        Return codebook indices for the given inputs (no EMA update, no loss).
        Used for transmitter bitstream encoding.

        Args:
            inputs: (B, C, H, W) in embedding space (e.g. after pre_vq_conv).

        Returns:
            indices: (N,) long tensor, N = B*H*W. Caller may reshape to (B, H, W).
        """
        with autocast(device_type=inputs.device.type, enabled=False):
            inputs = inputs.float()
            inputs = inputs.permute(0, 2, 3, 1).contiguous()
            flat_input = inputs.view(-1, self._embedding_dim)
            emb = self._embedding.weight.float()
            encoding_indices = self._nearest_embedding_indices(flat_input, emb)
        return encoding_indices
